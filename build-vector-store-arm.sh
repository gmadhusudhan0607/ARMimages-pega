#!/usr/bin/env bash
#
# Build ARM64 image for gen-ai-vector-store (includes service + ops + background)
#
# Prerequisites:
...
#
# Usage:
#   ./build-vector-store-arm.sh                          # Build and load locally
#   ./build-vector-store-arm.sh --push                   # Build and push to registry
#   ./build-vector-store-arm.sh --tag v1.0.0             # Custom image tag
#   ./build-vector-store-arm.sh --registry <ECR_URL>     # Custom registry
#
set -euo pipefail
...
REPO_NAME="gen-ai-vector-store-arm64"
...
VECTOR_STORE_ROOT="${SCRIPT_DIR}/gen-ai-vector-store-main/gen-ai-vector-store-main"
...
if [[ ! -d "${VECTOR_STORE_ROOT}/libs" ]]; then
    echo "==> Creating libs/ symlink for Docker build context..."
    ln -sfn "${VECTOR_STORE_ROOT}" "${VECTOR_STORE_ROOT}/libs"
fi

echo ""
echo "############################################################"
echo "#  Building gen-ai-vector-store ARM64 images"
echo "############################################################"

# Build genai-vector-store ARM64 image (main vector store service - port 8080)
build_image \
    "${REPO_NAME}" \
    "${VECTOR_STORE_ROOT}/distribution/service-docker/src/main/docker/Dockerfile" \
    "${VECTOR_STORE_ROOT}"

echo ""
echo "============================================================"
echo "  gen-ai-vector-store ARM64 images built successfully!"
echo ""
echo "  Image:"
