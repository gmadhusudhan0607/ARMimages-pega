...
var _ = Describe("Tests SVC Gemini 1.0 Pro Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("gemini-1.0-pro model tests", func() {

		XIt("when calling model gemini-1.0-pro, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			expectedPath := "/google/deployments/gemini-1.0-pro/chat/completions"
			expectedModelName := "gemini-1.0-pro"
			...
			requestUrl := fmt.Sprintf("%s/google/deployments/gemini-1.0-pro/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("google/gemini-1.0-pro", "v1", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 2048

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "gemini-1.0-pro", "gemini-1.0-pro", -1, expectedMaxOutputTokens, 1022)
