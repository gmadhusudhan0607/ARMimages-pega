...
func TestIntegration(t *testing.T) {
	...
	RunSpecs(t, "Service suite (retry on truncation)")
}
...
var serviceEnvVars = map[string]string{
	...
	"PRIVATE_MODEL_CONFIG_DIR": "test/integration/request-processing/retry_truncated/private-models-test-dir",

	"SERVICE_PORT":             strconv.Itoa(svcPort),
	"SERVICE_HEALTHCHECK_PORT": strconv.Itoa(svcHealthcheckPort),
	// No retry specific configuration needed - retry is built into the middleware
	// REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_IN_STREAMING defaults to false (streaming retry disabled)
