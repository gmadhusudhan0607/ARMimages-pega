...
var _ = Describe("Testing semantic search requests producing usage data", func() {
	var (
		ctx          context.Context
		isolationID  string
		collectionID string
	...
	_ = Context("semantic search usage metrics", func() {
		...
		When("PDC url is not specified", func() {
			It("it should not send data to PDC", func() {
				...
				docData := ReadTestDataFile("sem_search/DOC-1.json")
				...
				By("Performing semantic search query")
				queryEndpoint := fmt.Sprintf("%s/v1/%s/collections/%s/query/documents", svcBaseURI, isolationID, collectionID)
				queryData := `{
						"limit": 5,
						"maxDistance": 0.8,
						"filters": {
				...
				resp, _, err = HttpCall("POST", queryEndpoint, nil, queryData)
				Expect(err).To(BeNil())
				Expect(resp).NotTo(BeNil())
				Expect(resp.StatusCode).To(Equal(http.StatusOK))

				By("Waiting and verifying that PDC endpoint is NOT called")
		...
		When("PDC url is specified", func() {
			It("it should send data to PDC", func() {
				...
				By("Performing semantic search query on documents endpoint")
				queryEndpoint := fmt.Sprintf("%s/v1/%s/collections/%s/query/documents", svcBaseURI, isolationID, collectionID)
				queryData := `{
						"limit": 5,
						"maxDistance": 0.8,
				...
				By("Performing semantic search query on chunks endpoint")
				chunksQueryEndpoint := fmt.Sprintf("%s/v1/%s/collections/%s/query/chunks", svcBaseURI, isolationID, collectionID)

				resp, _, err = HttpCall("POST", chunksQueryEndpoint, nil, queryData)
				Expect(err).To(BeNil())
				Expect(resp).NotTo(BeNil())
				Expect(resp.StatusCode).To(Equal(http.StatusOK))

				By("Waiting and verifying that PDC endpoint IS called")
				...
				Expect(metric["metricType"]).To(Equal("SemanticSearchRequest"))
				Expect(metric).To(HaveKey("isolationID"))
				Expect(metric["isolationID"]).To(Equal(isolationID))
				Expect(metric).To(HaveKey("collectionID"))
				Expect(metric["collectionID"]).To(Equal(collectionID))
				Expect(metric).To(HaveKey("endpoint"))
				Expect(metric["endpoint"]).To(BeElementOf([]string{"query_documents", "query_chunks"}))
			})
		})
	})
})
