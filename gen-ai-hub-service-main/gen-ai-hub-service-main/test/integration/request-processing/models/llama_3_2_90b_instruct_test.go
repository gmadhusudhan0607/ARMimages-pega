...
var _ = Describe("Tests SVC Llama 3.2 90B Instruct Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("llama-3-2-90b-instruct model tests", func() {

		XIt("when calling model llama-3-2-90b-instruct, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			expectedPath := "/meta/deployments/llama-3-2-90b-instruct/chat/completions"
			expectedModelName := "llama-3-2-90b-instruct"
			...
			requestUrl := fmt.Sprintf("%s/meta/deployments/llama-3-2-90b-instruct/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("meta/llama-3-2-90b-instruct", "v1:0", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 2048

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "llama-3-2-90b-instruct", "llama-3-2-90b-instruct", -1, expectedMaxOutputTokens, 1022)
