...
var _ = Describe("Tests SVC Gemini 1.5 Flash Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("gemini-1.5-flash model tests", func() {

		It("when calling model gemini-1.5-flash, max_token must be set to fixed value if max_token was NOT provided in original request", func() {
			expectedPath := "/google/deployments/gemini-1.5-flash/chat/completions"
			expectedModelName := "gemini-1.5-flash"
			...
			requestUrl := fmt.Sprintf("%s/google/deployments/gemini-1.5-flash/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("google/gemini-1.5-flash", "002", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 8192

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "gemini-1.5-flash", "gemini-1.5-flash", -1, expectedMaxOutputTokens, 1022)
