...
var _ = Describe("Tests SVC GPT-4o Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("gpt-4o model tests", func() {
		...
		XIt("when calling model gpt-4o, max_token must be set to fixed value if max_token was NOT provided in original request", func() {
			...
			requestUrl := fmt.Sprintf("%s/openai/deployments/gpt-4o/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("openai/gpt-4o", "2024-11-20", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 16384

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "gpt-4o", "gpt-4o", -1, expectedMaxOutputTokens, 1022)
