Diagnose Vector Store issues - check isolations, database state, document processing, and SCE configuration.
...
Quick latency check - send a few query requests and analyze VS response headers to identify bottleneck. This is a lightweight version of the full `/perf-test` - useful during diagnostics to quickly spot if the problem is embedding, DB, or overhead.
...
> For a full test with percentiles, comparison mode, and report generation, use `/perf-test`.
...
| Slow queries (high latency) | Embedding, DB, or overhead bottleneck | Run Step 6 (Quick Perf Check) to identify dominant component. For full analysis use `/perf-test` |
