...
func TestIntegration(t *testing.T) {
	...
	RunSpecs(t, "Service suite (unrecognized models)")
}
...
var monitoringEventsPath = "/monitoring/events"
var mappingFile = "../mapping_20094.yaml"
var svcPort = 20090
var svcHealthcheckPort = 20092
var testDir = os.Getenv("PWD")

// Define environment variables directly in the test
// Used by test and service manager to run service in proper mode
var serviceEnvVars = map[string]string{
	"LOG_LEVEL": "DEBUG",

	// Four required configuration sources for NewTargetResolver
	"CONFIGURATION_FILE":       "test/integration/request-processing/mapping_20094.yaml",
	...
	"MONITORING_ENDPOINT":      fmt.Sprintf("%s%s", mockServerURL, monitoringEventsPath),
	"PRIVATE_MODEL_CONFIG_DIR": "test/integration/request-processing/unrecognized/private-models-test-dir",
...
var _ = BeforeSuite(func() {
	...
	// Create monitoring endpoint expectation before starting service
	err = functions.CreateMonitoringEndpointExpectation(mockServerURL, monitoringEventsPath)
	Expect(err).NotTo(HaveOccurred())
