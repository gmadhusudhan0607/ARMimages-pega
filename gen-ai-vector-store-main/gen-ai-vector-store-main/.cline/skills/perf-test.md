...
## Description
Runs a performance test against a Vector Store API endpoint - measures latency distribution (p50/p95/p99), throughput, and analyzes VS response headers for component-level breakdown (embedding, DB, overhead).

## Trigger
When user asks to:
- "perf test VS"
- "run performance test on Vector Store"
- "benchmark VS query"
- "send 300 requests to VS and check latency"
- "measure VS response times"
...
If port-forward is not active, set it up (same as E2E step 0) or tell the user to run the E2E test skill first.
...
- The target isolation and collection must exist and contain data. If they don't, offer Self-Contained Mode or tell the user to run the E2E test first (skip cleanup).
