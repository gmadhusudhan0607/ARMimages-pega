# Vector Store E2E Test

## Description
Runs a full end-to-end test of the Vector Store API on a live environment - from setup and isolation creation through document upsert, semantic query, and cleanup.

## Trigger
When user asks to:
- "run E2E test on VS"
- "test Vector Store end to end"
- "E2E on genai-int-upd"
- "test VS on <environment>"
- "full test of Vector Store"
...
```
Expected: **202 Accepted** (async processing with eventual consistency)
...
## Notes
- Test environments typically have `ISOLATION_ID_VERIFICATION_DISABLED=true` - VS accepts any isolation ID. We use a random UUID to ensure cleanup only removes our test data.
- `consistencyLevel=eventual` means async processing (202 response). Polling is required.
- The E2E test can be called standalone or as step 2.5 from the upgrade validation workflow.
