...
var _ = Describe("Tests SVC Nova Micro Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("nova-micro model tests", func() {

		XIt("when calling model nova-micro, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			expectedPath := "/amazon/deployments/nova-micro/chat/completions"
			expectedModelName := "nova-micro"
			...
			requestUrl := fmt.Sprintf("%s/amazon/deployments/nova-micro/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("amazon/nova-micro", "v1", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 10000

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "nova-micro", "nova-micro", -1, expectedMaxOutputTokens, 1022)
