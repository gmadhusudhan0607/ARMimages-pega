...
var _ = Describe("Tests SVC Claude 3.5 Sonnet Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("claude-3-5-sonnet model tests", func() {

		XIt("when calling model claude-3-5-sonnet, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			expectedPath := "/anthropic/deployments/claude-3-5-sonnet/chat/completions"
			expectedModelName := "claude-3-5-sonnet"
			...
			requestUrl := fmt.Sprintf("%s/anthropic/deployments/claude-3-5-sonnet/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("anthropic/claude-3-5-sonnet", "v2", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 8192

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "claude-3-5-sonnet", "claude-3-5-sonnet", -1, expectedMaxOutputTokens, 1022)
