...
var _ = Describe("Tests SVC GPT-4 Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("gpt-4 model tests", func() {

		XIt("when calling model gpt-4, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			...
			requestUrl := fmt.Sprintf("%s/openai/deployments/gpt-4/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("openai/gpt-4", "0613", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 4096

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "gpt-4", "gpt-4", -1, expectedMaxOutputTokens, 1022)
