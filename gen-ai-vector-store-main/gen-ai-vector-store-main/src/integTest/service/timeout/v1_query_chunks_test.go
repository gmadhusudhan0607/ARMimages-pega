...
var _ = Describe("POST /v1/{isolationID}/collections/{collectionName}/query/chunks", Ordered, func() {
	...
	_ = Context("calling service", func() {
		...
		_ = Context("timeout handling", func() {
			...
			It("returns chunks without retry when service responds quickly", func() {
				...
				By("Query chunks with test data - service should succeed without retry")
			...
			It("exhausts retries and returns gateway timeout when service is slow", func() {
				...
				By("Query chunks with test data - service should timeout and retry")
				queryData := ReadTestDataFile("test03/requests/query-chunks.json")
				resp, _, err := HttpCallWithHeadersAndApiCallStat("POST", endpointURI, ServerConfigurationHeaders, queryData)
				Expect(err).To(BeNil())
				Expect(resp).NotTo(BeNil())
			...
			It("succeeds after retry when initial request times out", func() {
				...
				By("Query chunks with test data - first call will timeout, retry will succeed")
				queryData := ReadTestDataFile("test03/requests/query-chunks.json")
				resp, _, err := HttpCallWithHeadersAndApiCallStat("POST", endpointURI, ServerConfigurationHeaders, queryData)
				Expect(err).To(BeNil())
				Expect(resp).NotTo(BeNil())
