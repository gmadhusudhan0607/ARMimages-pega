...
var _ = Describe("Tests SVC Gemini 1.5 Pro Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("gemini-1.5-pro model tests", func() {

		It("when calling model gemini-1.5-pro, max_token must be set to fixed value if max_token was NOT provided in original request", func() {
			expectedPath := "/google/deployments/gemini-1.5-pro/chat/completions"
			expectedModelName := "gemini-1.5-pro"
			...
			requestUrl := fmt.Sprintf("%s/google/deployments/gemini-1.5-pro/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("google/gemini-1.5-pro", "002", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 8192

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "gemini-1.5-pro", "gemini-1.5-pro", -1, expectedMaxOutputTokens, 1022)
