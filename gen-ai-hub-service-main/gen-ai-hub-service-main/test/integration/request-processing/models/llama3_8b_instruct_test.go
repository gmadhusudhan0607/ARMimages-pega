...
var _ = Describe("Tests SVC Llama3 8B Instruct Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("llama3-8b-instruct model tests", func() {

		XIt("when calling model llama3-8b-instruct, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			expectedPath := "/meta/deployments/llama3-8b-instruct/chat/completions"
			expectedModelName := "llama3-8b-instruct"
			...
			requestUrl := fmt.Sprintf("%s/meta/deployments/llama3-8b-instruct/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("meta/llama3-8b-instruct", "v1", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 2048

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "llama3-8b-instruct", "llama3-8b-instruct", -1, expectedMaxOutputTokens, 1022)
