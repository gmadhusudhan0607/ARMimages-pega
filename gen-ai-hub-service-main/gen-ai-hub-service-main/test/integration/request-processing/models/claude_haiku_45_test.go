...
var _ = Describe("Tests SVC Claude Haiku 4.5 Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("claude-haiku-4-5 model tests", func() {

		It("when calling model claude-haiku-4-5, model should be RECOGNIZED (not unrecognized) in metrics", func() {
			...
			expectedPath := "/anthropic/deployments/claude-haiku-4-5/chat/completions"
			expectedModelName := "claude-haiku-4-5"
			...
			// Call claude-haiku-4-5 model with simple request without max_tokens
			requestUrl := fmt.Sprintf("%s/anthropic/deployments/claude-haiku-4-5/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("anthropic/claude-haiku-4-5", "1.0", "").WithoutMaxTokens().Build()
			...
			max_tokens.CheckMetricsFixed(metricsUrl, testID, "claude-haiku-4-5", "claude-haiku-4-5", -1, expectedMaxOutputTokens, 1022)
		...
		It("when calling model claude-haiku-4-5, max_token must be set to fixed value if max_token was NOT provided in original request", func() {
			...
			// Call claude-haiku-4-5 model with simple request but without max_tokens in request
			requestUrl := fmt.Sprintf("%s/anthropic/deployments/claude-haiku-4-5/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("anthropic/claude-haiku-4-5", "1.0", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})
		...
		It("when calling model claude-haiku-4-5, max_token must not be changed if max_token was provided in original request", func() {
			...
			// Call claude-haiku-4-5 model with max_tokens in request
			requestUrl := fmt.Sprintf("%s/anthropic/deployments/claude-haiku-4-5/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("anthropic/claude-haiku-4-5", "1.0", "").WithMaxTokens(originalMaxTokens).Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 64000

			// Check all required metrics after calling service
			max_tokens.CheckMetricsFixed(metricsUrl, testID, "claude-haiku-4-5", "claude-haiku-4-5", originalMaxTokens, expectedMaxOutputTokens, float64(originalMaxTokens))
		...
		It("when calling model claude-haiku-4-5 with streaming, max_token must NOT be changed if max_token was NOT provided in original request (OutputTokensAdjustmentStreams=false)", func() {
			...
			// Call claude-haiku-4-5 model with streaming enabled but without max_tokens in request
			requestUrl := fmt.Sprintf("%s/anthropic/deployments/claude-haiku-4-5/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("anthropic/claude-haiku-4-5", "1.0", "").WithoutMaxTokens().WithStreaming(true).Build()
			...
			max_tokens.CheckMetricsStreaming(metricsUrl, testID, "claude-haiku-4-5", expectedModelName, 0, expectedMaxOutputTokens, 1022)
		...
		It("when calling model claude-haiku-4-5 with streaming, max_token must NOT be changed if max_token was provided in original request (OutputTokensAdjustmentStreams=false)", func() {
			...
			// Call claude-haiku-4-5 model with streaming enabled and max_tokens=768 in request
			requestUrl := fmt.Sprintf("%s/anthropic/deployments/claude-haiku-4-5/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("anthropic/claude-haiku-4-5", "1.0", "").WithMaxTokens(originalMaxTokens).WithStreaming(true).Build()
			headers := map[string]string{"Accept": "text/event-stream"}
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, headers)
			...
			// Check all required metrics after calling service
			max_tokens.CheckMetricsStreaming(metricsUrl, testID, "claude-haiku-4-5", "claude-haiku-4-5", originalMaxTokens, expectedMaxOutputTokens, 0)
