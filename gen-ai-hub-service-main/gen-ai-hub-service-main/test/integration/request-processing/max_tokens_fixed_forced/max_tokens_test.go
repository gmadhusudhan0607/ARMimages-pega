...
package max_tokens_fixed_forced_test
...
var _ = Describe("Tests SVC:", Ordered, func() {
	...
	_ = Context("REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED + FORCED", func() {
		...
		It("when calling model gpt-35-turbo with max_tokens lower than fixed value, it should be kept when forced=true", func() {
			// Define the original max_tokens value that will be sent in the request
			originalMaxTokens := 500
			expectedPath := "/openai/deployments/gpt-35-turbo-1106/chat/completions"
			expectedModelName := "gpt-35-turbo"
			// When FORCED=true and suggested (1022) >= original (500), keep original
			expectedMaxTokens := originalMaxTokens

			// Create WireMock expectation that validates max_tokens is kept at 500 (not forced to 1022)
			expectation, err := max_tokens.CreateWireMockMaxTokensExpectation(mockServerURL, testID, expectedPath, expectedModelName, expectedMaxTokens, 50)
			Expect(err).To(BeNil())
			testWireMockExpectations = append(testWireMockExpectations, expectation.Id)
			...
			// - genai_gateway_output_tokens_adjusted (must be 500 - kept original since suggested >= original)
			// - genai_gateway_output_tokens_maximum (must equal model's max from specs)
			// - genai_gateway_output_tokens_used (must be > 0)
			max_tokens.CheckMetricsFixedForced(metricsUrl, testID, "gpt-35-turbo", "gpt-35-turbo", originalMaxTokens, expectedMaxTokens, expectedMaxOutputTokens)
		...
		It("when calling model gpt-35-turbo with max_tokens higher than fixed value, it should be forced to fixed value when forced=true", func() {
			// Define the original max_tokens value that will be sent in the request
			originalMaxTokens := 2000
			expectedPath := "/openai/deployments/gpt-35-turbo-1106/chat/completions"
			expectedModelName := "gpt-35-turbo"
			// When FORCED=true and suggested (1022) < original (2000), force to suggested
			forcedMaxTokens := 1022 // The fixed value configured

			// Create WireMock expectation that validates max_tokens is forced to 1022 (from original 2000)
			expectation, err := max_tokens.CreateWireMockMaxTokensExpectation(mockServerURL, testID, expectedPath, expectedModelName, forcedMaxTokens, 50)
			Expect(err).To(BeNil())
			testWireMockExpectations = append(testWireMockExpectations, expectation.Id)
			...
			// Check all required metrics after calling service:
			// - genai_request_duration_ms (must be > 0)
			// - genai_gateway_output_tokens_requested (must be 2000 since max_tokens was in original request)
			// - genai_gateway_output_tokens_adjusted (must be 1022 - forced adjustment since suggested < original)
			// - genai_gateway_output_tokens_maximum (must equal model's max from specs)
			// - genai_gateway_output_tokens_used (must be > 0)
			max_tokens.CheckMetricsFixedForced(metricsUrl, testID, "gpt-35-turbo", "gpt-35-turbo", originalMaxTokens, forcedMaxTokens, expectedMaxOutputTokens)
