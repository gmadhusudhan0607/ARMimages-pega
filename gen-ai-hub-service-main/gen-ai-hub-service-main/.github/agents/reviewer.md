...
Code and configuration (including agent definitions in `.github/agents/`) must NEVER contain hardcoded HOME directory paths (e.g., `/Users/username/...`, `/home/username/...`, `~/.config/...`). Instead, paths should be resolved relative to the git repository root at runtime (e.g., using `$(git rev-parse --show-toplevel)`). Flag any hardcoded absolute paths that reference user home directories.
