...
var _ = Describe("Tests SVC Gemini 2.5 Pro Model Recognition (REQUEST_PROCESSING_OUTPUT_TOKENS_ADJUSTMENT_STRATEGY=FIXED):", Ordered, func() {
	...
	_ = Context("gemini-2.5-pro model tests", func() {

		XIt("when calling model gemini-2.5-pro, max_token must be set to fixed value if max_token was NOT provided in original request", func() { // temporary skipped
			expectedPath := "/google/deployments/gemini-2.5-pro/chat/completions"
			expectedModelName := "gemini-2.5-pro"
			...
			requestUrl := fmt.Sprintf("%s/google/deployments/gemini-2.5-pro/chat/completions", svcBaseURL)
			requestBody := max_tokens.NewLLMRequestBodyBuilder("google/gemini-2.5-pro", "001", "").WithoutMaxTokens().Build()
			max_tokens.ExpectLLMCall(testID, requestUrl, requestBody, map[string]string{})

			expectedMaxOutputTokens := 65536

			max_tokens.CheckMetricsFixed(metricsUrl, testID, "gemini-2.5-pro", "gemini-2.5-pro", -1, expectedMaxOutputTokens, 1022)
